...
export default function AdminProducts() {
  ...
  const loadProducts = async () => {
    ...
    try {
      ...
      setProducts(safeArr(response.data?.products));
    } catch (error) {
  ...
  const handleDuplicateProduct = (product) => {
    ...
    const variants = safeArr(product?.variants).length
      ? safeArr(product?.variants).map((v) => ({
          ...EMPTY_VARIANT,
          ...v,
          id: ensureVariantId({}), // new ids for copy
          stock: safeNum(v?.stock, 0),
          priceAdjustment: safeNum(v?.priceAdjustment, 0),
  ...
  // IMPORTANT: backend expects FULL Product object on PUT, not partial patch.
  ...
  const removeVariant = (index) => {
    ...
    setFormData({ ...formData, variants: newVariants.length ? newVariants : [] });
  };
  ...
  const filteredIds = filteredProducts.map((p) => p.id).filter(Boolean);
  const allSelected = filteredIds.length > 0 && selectedProducts.length === filteredIds.length;
